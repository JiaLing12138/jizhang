...
| 文件 | 定位 | 说明 |
|------|------|------|
| `记账OCR本地版.shortcut` | **日常正式版**（推荐） | 截屏 → 本地规则识别 → Base64 编码传入 Scriptable → 记账页预填确认。完全离线，不依赖任何网址 |
...
4. Scriptable 自动打开记账页并预填金额/分类/备注/日期 → 核对后点保存（无需任何权限弹窗）

## 工作方式

- 截屏 → 裁掉顶部状态栏 → 提取文字 → Base64 编码 → 打开 `scriptable:///run/记账?b64=<编码后的文字>`
- Scriptable 解码 URL 参数（剪贴板仅作兜底）→ 本地规则识别金额/分类/备注/日期/退款 → 预填表单待确认
